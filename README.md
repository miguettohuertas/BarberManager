...
App Mobile (Para Clientes): Focado na experiência do usuário (UX), permitindo cadastro ágil, escolha de serviços e agendamento de horários em poucos cliques.

Dashboard Web/Desktop (Para Administradores): Um painel de controle de alta produtividade (resolução 1280x720) para os barbeiros gerenciarem a fila de espera, serviços oferecidos e o faturamento diário.
...
Banco de Dados (Planejado): SQLite (Mobile) / Firebird ou MySQL (Servidor) com framework de persistência a ser definido.

📱 Telas e Navegação (Protótipo Interativo)

Atualmente, o projeto encontra-se na fase de Protótipo de Alta Fidelidade, onde todas as telas foram desenhadas e a navegação (roteamento) está 100% funcional, simulando a experiência final do usuário.
...
Calendário, grade de horários via FlowLayout e confirmação.
...
Nota: Para visualizar as imagens em alta resolução, acesse o diretório docs/telas/ neste repositório.
...
[x] Cards de Resumo Financeiro (KPIs: Faturamento, Ticket Médio, Pendentes).
...
[ ] Fase 5: Modelagem do Banco de Dados Relacional (DER).
...
Clique em "Entrar" para acessar a Home do cliente.

Na tela de Login, clique na Logo do aplicativo para acessar instantaneamente a área Administrativa (Desktop).
